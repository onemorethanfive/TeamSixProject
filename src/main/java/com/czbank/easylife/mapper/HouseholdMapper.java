...
//
    ...
    public Household findHouseholdByUserId(@Param("userId") String userId);

    @Select("SELECT * FROM household WHERE householdId = #{householdId} AND user_id = #{userId};")
    ...
    @Update("UPDATE household SET household_name = #{householdName}, household_num = #{householdNum}, household_loc = #{householdLoc};")
    public void updateHousehold(@Param("householdName")String householdName, @Param("householdNum")String householdNum, @Param("householdLoc")String householdLoc);
    //@Delete("DELETE FROM card_user_map WHERE card_id = #{cardId};")
    //    public void removeCardFromMap(@Param("cardId") String cardId);
    @Delete("DELETE from household WHERE householdId = #{householdId} AND user_id = #{userId} ;")
